...
while(True):
    ...
    data = json.load(urllib2.urlopen("http://www.thaiwaterqq.com/qq-web/api-web/getallconfig.php"))
    ...
    try:
        for i in range(len(data)):
            if(data[i]['mode']!=data_old[i]['mode']):
                ...
                print(masg)
                urllib2.urlopen("http://128.199.91.90/crflood/boat/maintain_botpush/botpush.php?stationid="+str(data[i]['station_id'])+"&modeold="+str(data_old[i]['mode'])+"&modenew="+str(data[i]['mode']))
    except:
        print("can't push noti")
    if(data !=""):
